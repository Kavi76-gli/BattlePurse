...
router.get("/profile", auth, async (req, res) => {
  try {
    ...
    const avatarUrl = user.avatar
      ? `https://battlepurse-2ch0.onrender.com/uploads/avatars/${user.avatar}`
      : null;
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const avatarUrl = `https://battlepurse-2ch0.onrender.com/uploads/avatars/${req.file.filename}`;
    ...
    const result = tournaments.map(t => {
      ...
      if (t.poster) {
        ...
        if (typeof t.poster === "string" && t.poster.startsWith("http")) {
          ...
          posterUrl = `https://battlepurse-2ch0.onrender.com/uploads/poster/${t.poster}`;
        }
      }
  ...
  try {
    ...
    res.json({
      ...
      posterUrl: tournament.poster
        ? `https://battlepurse-2ch0.onrender.com/uploads/${tournament.poster}`
        : null,
  ...
  try {
    ...
    const result = {
      ...
      poster: tournament.poster 
        ? `https://battlepurse-2ch0.onrender.com/uploads/${tournament.poster}` // ✅ return full image URL
        : null,
      roomId: joinedUser ? tournament.roomId : null,
      roomPassword: joinedUser ? tournament.roomPassword : null,
      joined: !!joinedUser
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const posterUrl = `https://battlepurse-2ch0.onrender.com/uploads/poster/${posterFileName}`;
  ...
  async (req, res) => {
    try {
      ...
      const imageUrls = imageFiles.map(f =>
        `https://battlepurse-2ch0.onrender.com/uploads/promos/${f}`
      );
  ...
  try {
    ...
    const result = promos.map(p => ({
      ...
      imageUrls: p.images.map(img =>
        img.startsWith("http")
          ? img // backward safety
          : `https://battlepurse-2ch0.onrender.com/uploads/promos/${img}`
      )
    }));
    ...
    res.json({
      ...
      results: (tournament.results || []).map(r => ({
        ...
        screenshot: r.screenshot
          ? `https://battlepurse-2ch0.onrender.com/uploads/${r.screenshot}` // ✅ Correct: serve actual file
          : null,
    ...
    const leaderboard = [1, 2, 3].map(pos => {
      ...
      if (w && w.userId) {
        ...
        const avatarUrl = w.userId.avatar
          ? `https://battlepurse-2ch0.onrender.com/uploads/avatars/${w.userId.avatar}`
...
router.post("/pair", authAdmin, async (req, res) => {
  try {
    ...
    for (const group of groups) {
      ...
      /* ---------- ROUNDS (NEGLECT DIFFERENCE) ---------- */
      let rounds = null;

      if (!isNonRoundGame) {
        const roundsList = group
          .map(p => Number(p.rounds))
          .filter(r => !isNaN(r));

        // Take minimum rounds so both players can play
        rounds = roundsList.length ? Math.min(...roundsList) : null;
      }
      ...
      const playersArr = group.map((p, idx) => ({
        ...
        joinedAt: new Date(),
        rounds: p.rounds || null, // keep player rounds individually
      ...
      const newMatch = new QuickMatch({
        ...
        prizeSystem,
        rounds, // final match rounds (minimum)
        players: playersArr,
        status: "paired",
        createdAt: new Date()
      });
    ...
    res.json({
      success: true,
      msg: "Players paired successfully (rounds neglected)",
      data: createdMatches
    });
...
// ✅ UNIVERSAL SINGLE ACTIVE MATCH ROUTE
    ...
    const userInfo = new Map(
      users.map(u => {
        const avatarUrl = u.avatar
          ? `https://battlepurse-2ch0.onrender.com/uploads/avatars/${u.avatar}`
          : null;
    ...
    const leaderboard = [...userMap.entries()]
      .map(([uid, data]) => {
        ...
        return {
          ...
          avatar: user.avatarUrl || "", // ✅ now full profile avatar
  ...
  async (req, res) => {
    try {
      ...
      const qrImageUrl = config.qrImage
        ? `https://battlepurse-2ch0.onrender.com/uploads/qr/${config.qrImage}`
        : null;
